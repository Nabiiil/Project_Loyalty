 ...
 * color alone). Colours are the validated categorical slots 1 (blue) and 2
 * (green), theme-aware via prefers-color-scheme.
 */
...
function fmtDay(iso: string): string {
  // Parse the YYYY-MM-DD as a plain date (no timezone shift) for the label.
...
export function NewVsReturningChart({ series }: { series: DayPoint[] }) {
  ...
  return (
    <div className="viz flex flex-col gap-3">
      ...
      <svg
        ...
        width="100%"
        role="img"
        aria-label={`New versus returning customers per day: ${totalNew} new, ${totalReturning} returning over ${n} days`}
        ...
        {series.map((p, i) => {
          ...
          return (
            <g key={p.day}>
              ...
              {/* Top segment: returning (rounded data-end) */}
              {retH > 0 && (
                <rect
                  x={x}
                  y={retY}
              ...
              {/* Whole-day hit target + native hover tooltip */}
              <rect x={i * slot} y={PAD_TOP} width={slot} height={PLOT_H} fill="transparent">
                <title>{`${fmtDay(p.day)}: ${p.new} new, ${p.returning} returning`}</title>
              </rect>
            </g>
          )
        })}
      ...
      {/* x-axis endpoints only — recessive, not one label per bar. */}
      {n > 0 && (
        <div className="flex justify-between text-[11px] text-zinc-400" style={{ marginTop: -8 }}>
          <span>{fmtDay(series[0].day)}</span>
          <span>{fmtDay(series[n - 1].day)}</span>
        </div>
      )}
    </div>
  )
