...
export async function EnrollmentCard({
  enrollment,
  isClaimed,
}: {
  ...
  const t = await getTranslations('enrollmentCard')
  const { current_stamps, businesses } = enrollment
  const name = businesses?.name ?? t('unknownBusiness')
  const threshold = businesses?.reward_threshold ?? 10
  // reward_description is owner-authored data; only the empty fallback is translated.
